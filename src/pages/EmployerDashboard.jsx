...
export default function EmployerDashboard() {
  ...
  const handleRefresh = async () => {
        await queryClient.invalidateQueries({ queryKey: ['employerShifts'] });
        await queryClient.invalidateQueries({ queryKey: ['all-applications'] });
                                  ...
                                  <div className="max-w-7xl mx-auto">
                                              ...
                                              <div className="mb-6 md:mb-12">
                                                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                                                                            <div>
                                                                              ...
                                                                              {venues.length > 1
                                                                                                    ? <Select value={selectedVenue?.id} onValueChange={(id) => {
                                    ...
                                    {/* Stats */}
                                              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-5 mb-6 md:mb-12">
                                                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-4 md:p-6 rounded-2xl hover-lift" style={{ backgroundColor: 'var(--warm-white)', border: '1px solid var(--sand)' }}>
                                                                            <Briefcase className="w-6 h-6 md:w-8 md:h-8 mb-2 md:mb-3" style={{ color: 'var(--sage)', strokeWidth: 1.5 }} />
                                                                            <div className="text-3xl md:text-4xl font-light mb-1 md:mb-2" style={{ fontFamily: 'Crimson Pro, serif', color: 'var(--earth)' }}>{availableShifts.length}</div>div>
                                                                            <div className="text-xs tracking-wider" style={{ color: 'var(--clay)' }}>OPEN SHIFTS</div>div>
                                                            </motion.div>motion.div>
                                                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="p-4 md:p-6 rounded-2xl hover-lift" style={{ backgroundColor: 'var(--terracotta)', color: 'white' }}>
                                                                            <Users className="w-6 h-6 md:w-8 md:h-8 mb-2 md:mb-3 opacity-90" style={{ strokeWidth: 1.5 }} />
                                                                            <div className="text-3xl md:text-4xl font-light mb-1 md:mb-2" style={{ fontFamily: 'Crimson Pro, serif' }}>{totalApplications}</div>div>
                                                                            <div className="text-xs tracking-wider opacity-90">APPLICATIONS</div>div>
                                                            </motion.div>motion.div>
                                                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="p-4 md:p-6 rounded-2xl hover-lift" style={{ backgroundColor: 'var(--sage)', color: 'white' }}>
                                                                            <Clock className="w-6 h-6 md:w-8 md:h-8 mb-2 md:mb-3 opacity-90" style={{ strokeWidth: 1.5 }} />
                                                                            <div className="text-3xl md:text-4xl font-light mb-1 md:mb-2" style={{ fontFamily: 'Crimson Pro, serif' }}>{filledShifts.length}</div>div>
                                                                            <div className="text-xs tracking-wider opacity-90">FILLED SHIFTS</div>div>
                                                            </motion.div>motion.div>
                                                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="p-4 md:p-6 rounded-2xl hover-lift" style={{ backgroundColor: 'var(--warm-white)', border: '1px solid var(--sand)' }}>
                                                                            <TrendingUp className="w-6 h-6 md:w-8 md:h-8 mb-2 md:mb-3" style={{ color: 'var(--olive)', strokeWidth: 1.5 }} />
                                                                            <div className="text-3xl md:text-4xl font-light mb-1 md:mb-2" style={{ fontFamily: 'Crimson Pro, serif', color: 'var(--earth)' }}>{completedShifts.length}</div>div>
                                                                            <div className="text-xs tracking-wider" style={{ color: 'var(--clay)' }}>COMPLETED</div>div>
                                                            </motion.div>motion.div>
                                              </div>div>
                                    ...
                                    {/* Tabs */}
                                              <Tabs defaultValue="shifts" className="space-y-8">
                                                            <TabsList className="grid w-full grid-cols-3 p-1.5 rounded-2xl h-auto" style={{ backgroundColor: 'var(--sand)' }}>
                                                                            ...
                                                                            <TabsTrigger value="analytics" className="rounded-xl py-2.5 font-normal text-sm tracking-wide transition-all data-[state=active]:shadow-none flex items-center justify-center gap-2" style={{ color: 'var(--earth)' }}>
                                                                                              ...
                                                                                              <span className="hidden sm:inline">Analytics</span>span>
                                                                            </TabsTrigger>TabsTrigger>
                                                            </TabsList>TabsList>
                                              
                                                            <TabsContent value="shifts" className="space-y-8">
                                                              {availableShifts.length > 0 && <RecommendedCandidates shift={availableShifts[0]} />}
                                                                                              ...
                                                                                              <CardContent>
                                                                                                {isLoading
                                                                                                                        ? <div className="grid md:grid-cols-2 gap-4">
                                                                                                                          {[...Array(4)].map((_, i) => <div key={i} className="h-48 rounded-xl animate-pulse" style={{ backgroundColor: 'var(--sand)' }} />)}
                                                                                                                          </div>div>
                                                                                                                    : shifts.length === 0
                                                                                                                      ? <div className="text-center py-12 rounded-xl" style={{ backgroundColor: 'var(--cream)' }}>
                                                                                                                                                  ...
                                                                                                                                                  <h3 className="text-xl font-normal mb-2" style={{ fontFamily: 'Crimson Pro, serif', color: 'var(--earth)' }}>No shifts posted yet</h3>h3>
                                                                                                                                                  <p className="font-light mb-4" style={{ color: 'var(--clay)' }}>Start by posting your first shift</p>p>
                                                                                                                                                  <Button onClick={() => setShowShiftForm(true)} className="rounded-xl font-normal" style={{ backgroundColor: 'var(--terracotta)', color: 'white' }}>
                                                                                                                                                                                <Plus className="w-4 h-4 mr-2" /> Post Shift
                                                                                                                                                    </Button>Button>
                                                                                                                        </div>div>
                                                                                                                      : <div className="grid md:grid-cols-2 gap-4">
                                                                                                                        {shifts.map((shift) =>
                                          <EmployerShiftCard
                                                            ...
                                                            <TabsContent value="jobs">
                                                                            <JobManagement venueId={selectedVenue?.id} venueType={selectedVenueType} />
                                                            </TabsContent>TabsContent>
                                              
                                                            <TabsContent value="analytics">
                                                                            <AnalyticsDashboard shifts={shifts} applications={allApplications} jobs={jobs} jobApplications={jobApplications} />
                                                            </TabsContent>TabsContent>
                                              </Tabs>Tabs>
                                  </div>div>
                        
                          {showShiftForm && selectedVenue &&
                                        <ShiftFormModal venue={selectedVenue} venueType={selectedVenueType} onClose={() => setShowShiftForm(false)} />
                          }
                          {viewingApplicationsFor &&
                                        <ApplicationsModal shift={viewingApplicationsFor} onClose={() => setViewingApplicationsFor(null)} />
                          }
                          {reviewShift &&
                                        <LeaveReviewModal shift={reviewShift} onClose={() => setReviewShift(null)} />
                          }
                          {viewingWorkerProfile &&
                                        <WorkerProfileModal workerEmail={viewingWorkerProfile} onClose={() => setViewingWorkerProfile(null)} />
                          }
                        </div>div>
                </>>
