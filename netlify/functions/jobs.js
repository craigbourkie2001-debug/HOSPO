...
export default async function handler(req) {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      const { venue_id, job_title, role_type, description } = body;
      if (!venue_id || !job_title || !role_type) {
        return new Response(JSON.stringify({ error: "venue_id, job_title, and role_type are required" }), { status: 400, headers: { "Content-Type": "application/json" } });
      }
      const [job] = await sql(`INSERT INTO jobs (venue_id, job_title, role_type, description) VALUES ($1,$2,$3,$4) RETURNING *`, [venue_id, job_title, role_type, description]);
    ...
    if (method === "PATCH" && id) {
      ...
      const [updated] = await sql(`UPDATE jobs SET ${setClauses}, updated_at = NOW() WHERE id = $1 RETURNING *`, [id, ...Object.values(body)]);
      return new Response(JSON.stringify(updated), { status: 200, headers: { "Content-Type": "application/json" } });
    }

    if (method === "DELETE" && id) {
      await sql(`DELETE FROM jobs WHERE id = $1`, [id]);
